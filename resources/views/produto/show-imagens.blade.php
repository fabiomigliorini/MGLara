            ...
            @foreach($imagens as $i => $imagem)
                <div class="item text-center {{ ($i == 0)?'active':'' }}">
                    ...
                    <div class="carousel-caption">
                        ...
                        <p>
                            <a href='{{ url("imagem/produto/$model->codproduto?imagem={$imagem->codimagem}") }}'><i class="glyphicon glyphicon-pencil"></i></a>
                            <a href="{{ url("imagem/produto/{$model->codproduto}/delete?imagem={$imagem->codimagem}") }}" data-excluir data-pergunta="Tem certeza que deseja excluir a imagem '{{ $imagem->observacoes }}'?" data-after-delete="recarregaDiv('div-imagens');"><i class="glyphicon glyphicon-trash"></i></a>
                        </p>
                    </div>
                </div>
            @endforeach
