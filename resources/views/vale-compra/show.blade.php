        ...
        <li class='active'>
            <small>
                ...
                @if (empty($model->inativo))
                    <a title='Inativar' href="{{ url('vale-compra/inativar') }}" data-inativar
                        data-codigo="{{ $model->codvalecompra }}" data-acao="inativar"
                        data-pergunta="Tem certeza que deseja inativar o vale {{ formataCodigo($model->codvalecompra) }}? "
                        data-after-inativar="location.reload()"><span class="glyphicon glyphicon-ban-circle"></span></a>
                    &nbsp;
                    <a title='Impressão' href="#" data-toggle="modal" data-target="#modalRelatorio"
                        id="linkImpressao"><span class="glyphicon glyphicon-print"></span></a>
                @endif
                    ...
                    <li class='list-group-item'>
                        <div class='row'>
                            ...
                            <div class='col-md-5'>
                                ...
                                @if (!empty($inativo))
                                    <s><a
                                            href='{{ url('produto', $vcmpb->ProdutoBarra->codproduto) }}'>{{ $vcmpb->ProdutoBarra->descricao() }}</a></s>
            ...
            <ul class="list-group list-group-condensed list-group-hover list-group-striped" id='divListagemProdutos'>
                        ...
                        @foreach ($model->ValeCompraFormaPagamentoS as $pag)
                            <div class='col-md-8'>
                                ...
                                @foreach ($pag->TituloS as $titulo)
                                    <br>
                                    <a href="{{ env('MGSIS_URL') }}/index.php?r=titulo/view&id={{ $titulo->codtitulo }}"
                                        target='_blank'>
                                        {{ $titulo->numero }}
                                    </a>
                ...
                <li class='list-group-item'>
                    <div class='row'>
                        ...
                        <div class='col-md-8'>
                            <a href="{{ env('MGSIS_URL') }}/index.php?r=titulo/view&id={{ $model->codtitulo }}"
                                target='_blank'>
                                {{ $model->Titulo->numero }}
                            </a>
                            <small class="pull-right text-muted">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    <iframe id="frameImpressao" name="frameImpressao" src=""
...
@section('inscript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            @if ($imprimir == true)
                $('#frameImpressao').attr('src',
                    '{{ url("vale-compra/{$model->codvalecompra}/imprimir?imprimir=true") }}');
                $('#modalRelatorio').modal('show')
            @endif

            $('#linkImpressao').click(function(e) {
                $('#frameImpressao').attr('src',
                    '{{ url("vale-compra/{$model->codvalecompra}/imprimir?imprimir=false") }}');
            });
